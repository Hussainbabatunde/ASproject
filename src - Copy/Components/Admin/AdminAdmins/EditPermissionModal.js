...
const EditAdminPermissionModal = ({show, name, describe, fetcher, loader, handleSubmit, handleChange, title, handleHide}) => {
  ...
  >
    <div className='AdminPermissionModal'>
        <form onSubmit={handleSubmit} className='AdminPermissionModal_body'>
            ...
            {fetcher == true ? <Skeleton variant="rounded" width='90%' height={32} />:<input type='text' value={name?.name} className='AdminPermissionModal_Input' name='name' onChange={handleChange} />}
            <p className='AdminPermissionModal_Inputlabel'>Description</p>
            {fetcher == true ? <Skeleton variant="rounded" width='90%' height={32} />:<input type='text' value={name?.description} className='AdminPermissionModal_Input' name='description' onChange={handleChange} />}
