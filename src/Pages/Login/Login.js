    ...
    <div className='Loginpage'>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='Loginpage_formSection'>
            ...
            <div className='CreateAccpage_formInputfield'>
                ...
                {showPassword == 'password'&& <RxEyeClosed onClick={displayPassowrd} style={{fontSize: '16px'}} />}
                {showPassword == 'text' && <BsEyeFill onClick={hidePassowrd} style={{fontSize: '16px'}}/>}
                </div>
                {errors?.password && <p className='error_authentication'>Password is required</p>}
            </div>
            <p className='Loginpage_formForgotPassword'><Link style={{color:'#0D0D9C'}} to='/forgotPassword'>Forgot Password?</Link></p>
            <button className='Loginpage_formLoginButton' onClick={handleSubmit}>Log in</button>
            ...
            <p className='Loginpage_formSignup'>Don't have an account?  <span className='Loginpage_SingupText'><Link className='Loginpage_enquireacc' to='/signup'>Sign up</Link></span></p>
            <Modal
        open={show}
        // onClose={handleClose}
